...
def shift_left(img, img_col, img_row, num_sft):
    img_tmp = img.reshape((img_col, img_row))
    img_tmp = np.delete(img_tmp, np.s_[0:num_sft], axis=1)
    img_tmp = np.column_stack((img_tmp, np.zeros((img_col, num_sft)))).reshape((img_col*img_row,))
    return img_tmp
...
def main_vgg(i_img=27, num_sft=10, save=False):
    img1 = load_data(i_img)
    img1 = data_preprocess(img1)
    img_col, img_row = 60, 12
    # shift left pixels
    img1_sft = shift_left(img1, img_col, img_row, num_sft)
    print('shift {}px left in img1'.format(num_sft))
    # plot image
    plt.subplot(121)
...
def simi_loop(i_img=27, num_sft=6, save=False):
    simi_720_lst, simi_4096_lst = [], []
    for i in range(num_sft):
        simi_720, simi_4096 = main_vgg(i_img, num_sft=i)
        simi_720_lst.append(simi_720)
        simi_4096_lst.append(simi_4096)
    # plot similarity scatter plots
    plt.figure(figsize=(10,4))
    plt.scatter(range(num_sft), simi_720_lst, c='blue', label='w/out vgg')
    plt.scatter(range(num_sft), simi_4096_lst, c='red', label='w/ vgg')
    plt.xticks(np.arange(0,num_sft,1))
    plt.yticks(np.arange(0, 1.2, .2))
    plt.xlabel('# of left-shifting pixels')
...
simi_720_lst_27, simi_4096_lst_27 = simi_loop(i_img=226, num_sft=7)